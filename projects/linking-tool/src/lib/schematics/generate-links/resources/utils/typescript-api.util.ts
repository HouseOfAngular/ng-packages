...
export class TypescriptApiUtil {
  ...
  static getStringValue(propertyAssignment: PropertyAssignment): string {
    if (
      CommonUtil.isFunction(
        (propertyAssignment.getInitializer() as StringLiteral).getLiteralValue
      )
    ) {
  ...
  static isArrayValue(propertyAssignment: PropertyAssignment): boolean {
    return CommonUtil.isFunction(
      TypescriptApiUtil.getArrayLiteralFromPropertyAssignment(
        propertyAssignment
      ).getElements
    );
  }
  ...
  static isArrowFunction(propertyAssignment: PropertyAssignment): boolean {
    return (
      ...
      (propertyAssignment.getInitializer() as Expression).getKind() ===
  ...
  static isVariableDeclarationExisting(
    ...
    return (
      ...
      !!(source.getVariableDeclaration(
        property
      ) as VariableDeclaration).getInitializer()
    );
  }
  ...
  static getCallExpressionFromArrowFunction(
